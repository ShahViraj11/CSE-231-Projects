        ...
        if list_1 is None: 
            ...
            if len(list_1) == 1: 
                ...
                if list_1[0].upper() == 'WT':
                    var_2 = waste_to_tableau(waste1, tableau1, int(list_1[1]) - 1) #moving card from waste to tableau by calling the respective function
                    if not var_2: #checking if move is valid or not
                        print("\nInvalid move!\n") 
                    display(tableau1, stock1, foundation1, waste1)  # calling the display function
                elif list_1[0].upper() == 'WF':
                    var_3 = waste_to_foundation(waste1, foundation1, int(list_1[1]) - 1) #moving card from waste to foundation by calling the respective function
                    if not var_3:
                        print("\nInvalid move!\n")
                    g_won = check_win(stock1, waste1, foundation1, tableau1) #checking if the user won as foundation is invloved
                    if g_won:
                        print("You won!")
                    display(tableau1, stock1, foundation1, waste1)  # calling the display function
            elif len(list_1) == 3:  #if the length of list being 2
                if list_1[0].upper() == 'TT':
                    var_4 = tableau_to_tableau(tableau1, int(list_1[1]) - 1, int(list_1[2]) - 1) #moving card from tableau to tableau by calling the respective function
                    if not var_4:
                        print("\nInvalid move!\n")
                    display(tableau1, stock1, foundation1, waste1)  # calling the display function
